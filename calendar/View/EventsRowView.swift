...
final internal class EventsRowView: UIScrollView, ReusableObject {
    ...
    private func commonInit() {
        contentSize   = CGSize(width: frame.width, height: 300)
        clipsToBounds = true
    ...
    func reload() {
        ...
        eventRanges.sorted(by: { (first, second) -> Bool in
            ///
            return first.key.section < second.key.section || first.key.row < second.key.row
                ...
                for i in 0..<lines.count {
                    let indexes = lines[i]
                    if !indexes.intersects(integersIn: Range.init(range)!) {
                        numLine = i
                        break
                    }
                }
                if numLine == -1 {
                    numLine = lines.count
                    lines.append(IndexSet(integersIn: Range.init(range)!))
                } else {
                    lines[numLine].insert(integersIn: Range.init(range)!)
                }
    ...
    func cellsInRect(_ rect: CGRect) -> [EventView] {
        var rows: [EventView] = []
    ...
    func cellAtIndexPath(_ indexPath: IndexPath) -> EventView? {
    ...
    func didTapCell(_ cell: EventView, atIndexPath indexPath: IndexPath) {
        if cell.selected {
